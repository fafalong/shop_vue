<template>
    <div class="order-wrap">
        ...
        <section class="order-tag">
            ...
            <span :class="{'active' : orderType===3}" data-type="3" @click="selectTag">待发货</span>
            <span :class="{'active' : orderType===4}" data-type="4" @click="selectTag">待收货</span>
            <span :class="{'active' : orderType===5}" data-type="5" @click="selectTag">已完成</span>
        </section>
            ...
            <div class="order-list" v-show="!isLoading">
                <div class="order-item" v-for="order in orderList" :data-orderNo="order.orderNo" @click="viewDetail">
                    ...
                    <div class="status">
                      ...
                      <span class="order-payment" v-show="order.status === 30" @click="getProduct(order.orderNo)">确认收货</span>
...
<script>
    ...
    export default {
        data() {
            return {
                orderList: [], //所有订单
                orderType: 1,  //1-全部  2-待付款  3-待发货  4-待收货  5-已完成
                emptyMsg: '',
                isLoading: true
            }
        },
      ...
      mounted(){
        ...
        methods: {
            ...
            //确认收货
            getProduct(orderNo){
              this.$http('/api/order/getProduct.do',{
                orderNo: orderNo
              },'POST').then((res)=>{
                if(res.data==true){
                  alert("该订单已完成，祝您生活愉快")
                  this.$router.replace('./order-list?orderType='+5)
                  this.orderList = []
                  this.emptyMsg = ''
                  this.getOrderList()
                }
